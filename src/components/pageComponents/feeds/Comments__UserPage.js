...
function ShetsUserPage({me, setOverlayImage, setOverlayVisibility, setOverlayImages}) {
    const { username } = useParams();
    const [startPos, setStartPos] = useState(0);
    const setStartPosfromChild = (startPosFromChild) =>{
        setStartPos(startPosFromChild)
    }
    const {loading, error, Data, hasMore} = PostsLoader(startPos, 14, setStartPosfromChild, username);
    ...
    return (
        <>
            {Data.length === 0 ? <div>This user hasn't commented anything yet</div> :""}
            ...
            {Array.from(Data).map((post,index) => 
                <Link key={"c"+post.id} to={"/comment/" + post.id}>
                    {index === Data.length - 6 ?
                    <div ref={tenthPost}>
                        <Post post={post} isPost={false} me={me} setOverlayImage={setOverlayImage} setOverlayVisibility={setOverlayVisibility}
