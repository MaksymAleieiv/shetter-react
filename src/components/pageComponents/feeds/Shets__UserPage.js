...
function ShetsUserPage({me, setOverlayImage, setOverlayVisibility, setOverlayImages}) {
    ...
    const {loading, error, Data, hasMore} = PostsLoader(startPos, 1, setStartPosfromChild, username);
    ...
    return (
        <>
            {Data.length === 0 ? <div>This user hasn't posted anything yet</div> :""}
            ...
            {Array.from(Data).map((post,index) => 
                <Link key={"p"+post.id} to={"/post/" + post.id}>
                    {index === Data.length - 6 ?
                    <div ref={tenthPost}>
